	...
	return (
		<>
			<form className='w-full my-2 flex justify-center'>
				<select className="w-3/12 px-8 py-3 rounded-l-lg font-medium bg-gray-100 border border-gray-200 placeholder-gray-500 text-sm focus:outline-none focus:border-gray-400 focus:bg-white" value={search} onChange={(e) => { setSearch(e.target.value) }} onSubmit={fetchData}>
					{
				...
				<button ref={searchBtn} name='search' className="tracking-wide font-semibold bg-indigo-500 text-gray-100 w-40 py-3 rounded-r-lg hover:bg-indigo-700 transition-all duration-500 ease-in-out flex items-center justify-center focus:shadow-outline focus:outline-none"
